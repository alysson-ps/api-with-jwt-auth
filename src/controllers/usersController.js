"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.resetPassword = exports.forgotPassword = exports.registerUser = exports.loginUser = void 0;
var jsonwebtoken_1 = __importDefault(require("jsonwebtoken"));
var crypto_1 = __importDefault(require("crypto"));
var mailer_1 = __importDefault(require("../utils/mailer"));
var connetions_1 = __importDefault(require("../database/connetions"));
var secret_1 = require("../utils/secret");
var loginUser = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, email, username, password, user, id, token;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _a = req.body, email = _a.email, username = _a.username, password = _a.password;
                if (email == null && username == null && password == null) {
                    return [2 /*return*/, res.status(400).json({ error: "Data sent is empty" })];
                }
                return [4 /*yield*/, connetions_1.default.table("users").where("email", "=", email).first()];
            case 1:
                user = _b.sent();
                if (user) {
                    if (user.password === password) {
                        id = user.id;
                        token = jsonwebtoken_1.default.sign({ id: id }, secret_1.secret, {
                            expiresIn: 18000,
                        });
                        return [2 /*return*/, res.status(200).json({ token: token })];
                    }
                    else {
                        return [2 /*return*/, res.status(401).json({ error: "Not Unauthorized" })];
                    }
                }
                else {
                    return [2 /*return*/, res.status(400).json({ error: "User Not found" })];
                }
                return [2 /*return*/];
        }
    });
}); };
exports.loginUser = loginUser;
var registerUser = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, email, username, password, user, now, users;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _a = req.body, email = _a.email, username = _a.username, password = _a.password;
                if (email == null && username == null && password == null) {
                    return [2 /*return*/, res.status(400).json({ error: "Data sent is empty" })];
                }
                return [4 /*yield*/, connetions_1.default.table("users").where("email", "=", email).first()];
            case 1:
                user = _b.sent();
                if (user) {
                    return [2 /*return*/, res.status(400).json({ error: "Email already exist" })];
                }
                now = new Date();
                return [4 /*yield*/, connetions_1.default.table("users").insert({
                        email: email,
                        username: username,
                        password: password,
                        created_at: now,
                        updated_at: now,
                    })];
            case 2:
                _b.sent();
                return [4 /*yield*/, connetions_1.default.table("users").where("email", "=", email).first()];
            case 3:
                users = _b.sent();
                return [2 /*return*/, res.json({ users: users })];
        }
    });
}); };
exports.registerUser = registerUser;
var forgotPassword = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var email, user, ExpireIn, token;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                email = req.body.email;
                return [4 /*yield*/, connetions_1.default.table("users").where("email", "=", email).first()];
            case 1:
                user = _a.sent();
                if (!user)
                    return [2 /*return*/, res.status(400).json({ error: "User Not found" })];
                ExpireIn = new Date();
                ExpireIn.setHours(ExpireIn.getHours() + 1);
                token = crypto_1.default.randomBytes(20).toString("hex");
                return [4 /*yield*/, connetions_1.default
                        .table("users")
                        .update({
                        password_reset_token: token,
                        password_reset_token_expire: ExpireIn,
                    })
                        .where("email", "=", email)];
            case 2:
                _a.sent();
                mailer_1.default(email, token)
                    .then(function (result) {
                    return res.json({ message: result });
                })
                    .catch(function (err) {
                    return res.json({ message: err });
                });
                return [2 /*return*/];
        }
    });
}); };
exports.forgotPassword = forgotPassword;
var resetPassword = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, email, password, token, user, now;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _a = req.body, email = _a.email, password = _a.password, token = _a.token;
                return [4 /*yield*/, connetions_1.default.table("users").where("email", "=", email).first()];
            case 1:
                user = _b.sent();
                now = new Date();
                if (!user)
                    return [2 /*return*/, res.status(400).json({ error: "User Not found" })];
                if (user.password_reset_token !== token)
                    return [2 /*return*/, res.status(400).json({ error: "Token invalid" })];
                if (now > user.password_reset_token_expire)
                    return [2 /*return*/, res.status(400).json({ error: "Token expired" })];
                return [4 /*yield*/, connetions_1.default.table("users").where("email", "=", email).update({
                        password: password,
                    })];
            case 2:
                _b.sent();
                return [2 /*return*/, res.status(200).json({ message: "ok" })];
        }
    });
}); };
exports.resetPassword = resetPassword;
